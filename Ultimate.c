...
int main() {
  ...
  do {
    ...
    /*INCLUYE EL 0 EN LA PILA SI EL USUARIO DIGITA EL CERO*/
    if (strcmp(n,"0") == 0) {
      Push(&pila, 0);
    }else
    //PARA QUE AL LEER ENTER "\0"
    //Y QUE LA CONVERSION DE LA CADENA A FLOAT SEA DISTINTA A CERO, YA QUE DARIA
    //PROBLEMAS AL INGRESAR CARACTERES/PALABRAS DISTINTAS DE LAS QUE ESTAN DISPONIBLES EN LA
    //CALCULADORA
    if (n[0] != '\0' && atof(n) != 0) {
      Push(&pila, atof(n));
    }
